    ...
    public static class BinarySerializer
    {
        ...
        private static byte[] SerializeField(FieldInfo field, object targetObject)
        {
            ...
            if (fieldType.IsClass)
            {
                ...
                if (fieldValue is SerializableObjectMono)
                {
                    SerializableObjectMono som = (SerializableObjectMono)fieldValue;
                    byte[] data = BitConverter.GetBytes(som.NetworkID);
                    return data;
                }
            }
                        ...
                        if (arr.Rank == 1)
                        {
                            ...
                            else if (elementType.IsClass)
                            {
                                for (int i = 0; i < len; i++)
                                {
                                    ...
                                    {
                                        SerializableObjectMono som = (SerializableObjectMono)element;
        ...
        public static void Deserialize(SerializableObjectMono obj, byte[] data)
        {
            Type type = obj.GetType();
            FieldInfo[] fieldInfos = type.GetFields(BindingFlags.Public | BindingFlags.Instance | BindingFlags.NonPublic);
            List<FieldInfo> fieldInfoList = new List<FieldInfo>(fieldInfos);
